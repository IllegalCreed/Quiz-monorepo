#!/usr/bin/env node
// Create dev/test/prod databases and users on a MySQL-compatible server (e.g., RDS)
// Usage:
...
const rootUser = process.env.DB_ROOT_USERNAME || process.env.DATABASE_USERNAME || 'root';
const rootPassword = process.env.DB_ROOT_PASSWORD || process.env.DATABASE_PASSWORD || '';

const devDb = process.env.DEV_DATABASE_NAME || 'quiz_dev';
const devUser = process.env.DEV_DATABASE_USER || 'quiz_dev_user';
const devPwd = process.env.DEV_DATABASE_PASSWORD || crypto.randomBytes(12).toString('base64');

const testDb = process.env.TEST_DATABASE_NAME || 'quiz_test';
const testUser = process.env.TEST_DATABASE_USER || 'quiz_test_user';
const testPwd = process.env.TEST_DATABASE_PASSWORD || crypto.randomBytes(12).toString('base64');

const prodDb = process.env.PROD_DATABASE_NAME || 'quiz_prod';
const prodUser = process.env.PROD_DATABASE_USER || 'quiz_prod_user';
const prodPwd = process.env.PROD_DATABASE_PASSWORD || crypto.randomBytes(12).toString('base64');
...
async function ensure(conn, db, user, pwd) {
  ...
  await conn.query(`CREATE USER IF NOT EXISTS \`${user}\`@'%' IDENTIFIED BY ?;`, [pwd]);
...
(async function main() {
  if (!rootPassword) {
    console.error('DB root password is required. Set DB_ROOT_PASSWORD or DATABASE_PASSWORD in your .env.create-db.local.');
    process.exit(1);
  }
