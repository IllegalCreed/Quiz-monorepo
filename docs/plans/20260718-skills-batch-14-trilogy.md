...
- [x] **生产完成（build 一次流程，分类树批 1 已建，无需迁移）**：import 79 题到 RDS → 查真实 ID（Hugging Face 656 / Gemini 657 / DeepMind Science 658 / AI 论文复现 659；组节点「AI/ML 与科研工作流」= 655，批 1 已建）回填 4 页 index.md + sidebar 新建组 → **VitePress build 唯一一次**（797s 真实成功）→ 提交推送 VitePress（单 commit）→ rsync 部署笔记（dry-run 确认 0 误删 SlideStack）+ 4 幻灯片 → **全 12 路 HTTP 200 上线**（4 笔记 + 4 幻灯片 + 4 测试题接口，2026-07-18）
