...
6. ~~生产部署~~ ✅ — Nginx HTTPS 反向代理 + PM2 + 一键部署脚本 + 同意后的 GA4/百度页面浏览
...
7. ~~SSE 客户端管理~~ ✅
8. ~~Dashboard 数据可视化~~ ✅
9. ~~生产部署 + 安全加固 + basic consent 后的 GA4/百度页面浏览~~ ✅
...
> 非代码的方案设计文档统一存放在 `docs/plans/` 目录下。

### 存放规则

- **位置**：`docs/plans/{方案名}.md`
- **命名**：kebab-case，简明描述方案主题，如 `content-production-workflow.md`
- **时机**：在开始非平凡的实现任务之前，先在此目录创建方案文档，获得确认后再动手
- **内容**：包含 Context（为什么做）、具体实施步骤、涉及的关键文件路径、验证方式
- **生命周期**：方案实施完成后保留存档，不删除；如方案废弃则在文件顶部标注 `> ⚠️ 已废弃`

### 现有计划

| 文件                                                                     | 说明                                                       |
| ------------------------------------------------------------------------ | ---------------------------------------------------------- |
| [content-production-workflow.md](./plans/content-production-workflow.md) | 内容生产流程（VitePress 笔记 + Slidev 幻灯片 + Quiz 题目） |
